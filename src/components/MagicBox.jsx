...
const MagicBox = ({ onCommand, onMapFocus }) => {
    ...
    return (
        <div className="magic-box-container" style={{ position: 'fixed', bottom: '30px', left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'flex-end', gap: '16px', zIndex: 4000 }}>
            ...
            <div style={{ position: 'relative' }}>
                <AnimatePresence>
                    {showSmartCmdMenu && (
                        <motion.div ref={smartMenuRef} initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="glass-card" style={{ position: 'absolute', bottom: '70px', left: '0', width: '280px', maxHeight: '380px', padding: '12px', background: 'rgba(12, 12, 14, 0.98)', border: '1px solid var(--accent-color)', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--accent-color)', marginBottom: '8px', display: 'flex', justifyContent: 'space-between' }}>
                                <span>✨ 常用快捷指令</span>
                            </div>
                            {/* 指令列表 */}
                            {smartCommands.sort((a, b) => (b.pinned ? 1 : 0) - (a.pinned ? 1 : 0)).map((cmd, i) => (
                                <div key={i} className="menu-item" style={{ padding: '8px 10px', borderRadius: '8px', background: cmd.pinned ? 'rgba(99, 102, 241, 0.08)' : 'transparent', fontSize: '13px', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', transition: '0.2s' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        <Pin size={12} onClick={(e) => togglePinCommand(i, e)} style={{ cursor: 'pointer', color: cmd.pinned ? 'var(--accent-color)' : 'rgba(255,255,255,0.2)' }} />
                                        <Trash2 size={12} onClick={(e) => deleteSmartCommand(i, e)} style={{ cursor: 'pointer', color: 'rgba(255,100,100,0.5)' }} />
                                    </div>
                                </div>
                            ))}
                            {/* 添加区域 */}
                            <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid var(--glass-border)', display: 'flex', gap: '8px' }}>
                                <input value={newCmdText} onChange={(e) => setNewCmdText(e.target.value)} placeholder="添加新指令..." style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '6px', padding: '6px 8px', fontSize: '12px', color: 'white' }} />
                                <button onClick={addSmartCommand} style={{ background: 'var(--accent-color)', border: 'none', borderRadius: '6px', width: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white' }}><Plus size={16} /></button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            <div style={{ position: 'relative' }}>
                <AnimatePresence>
                    {showFunctionMenu && (
                        <motion.div ref={funcMenuRef} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="glass-card" style={{ position: 'absolute', bottom: '70px', left: '0', width: '200px', padding: '8px', background: 'rgba(12, 12, 14, 0.98)', border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            {functions.map(fn => (
                                <div key={fn.id} onClick={() => handleFunctionSelect(fn.id)} className="menu-item" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', borderRadius: '8px', cursor: 'pointer', transition: '0.2s', background: activePanel === fn.id ? 'rgba(255,255,255,0.05)' : 'transparent' }}>
                ...
                <motion.div layout className="glass-card" style={{ width: '500px', height: '54px', borderRadius: '16px', display: 'flex', alignItems: 'center', padding: '0 12px 0 8px', gap: '10px', background: isFocused ? 'rgba(12, 12, 14, 0.95)' : 'rgba(12, 12, 14, 0.7)', border: isFocused ? '1px solid var(--accent-color)' : '1px solid var(--glass-border)' }}>
                    <div ref={funcBtnRef} onClick={() => setShowFunctionMenu(!showFunctionMenu)} style={{ height: '36px', padding: '0 10px 0 8px', background: activePanel ? 'rgba(99, 102, 241, 0.15)' : 'rgba(255,255,255,0.05)', border: activePanel ? '1px solid var(--accent-color)' : '1px solid var(--glass-border)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', transition: '0.2s', minWidth: '110px' }}>
                        <div style={{ color: activePanel ? 'var(--accent-color)' : 'white' }}>{currentFunction ? currentFunction.icon : <Sparkles size={16} />}</div>
                        <span style={{ fontSize: '13px', fontWeight: 500, color: 'white', flex: 1 }}>{currentFunction ? currentFunction.label : '功能模式'}</span>
                    ...
                    <form onSubmit={handleSend} style={{ flex: 1 }}>
                        <input value={input} onChange={(e) => setInput(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} placeholder={activePanel ? "实时修改面板内容..." : "对话..."} style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', color: 'white', fontSize: '14px' }} />
                    </form>
